    ...
    while True:
        ...
        if op == 1:
            ...
            if not resultados:
                ...
                print(
                    '(id_evento,nome,data,valor,artistas,informacoes,link_site_vendas,id_genero,id_organizador,endereco)')
        ...
        # Inserção
            ...
            while True:
                ...
                cursor.execute("SELECT * FROM Evento_Artista WHERE id_evento = %s AND id_artista =%s",
        ...
        # Listar usuários que vão em cada evento
            ...
            cursor.execute(
                "SELECT usuario.nome FROM Usuario_Eventos JOIN Usuario ON Usuario_Eventos.id_usuario = Usuario.id_usuario WHERE Usuario_Eventos.id_evento = %s;",
                (id_evento,))
        ...
        # Listar todos os artistas de um evento
            ...
            cursor.execute(
                "SELECT Artista.nome FROM Evento_Artista JOIN Artista ON Evento_Artista.id_artista = Artista.id_artista WHERE Evento_Artista.id_evento = %s;",
                (id_evento,))
            artistas = cursor.fetchall()
            if not artistas:
