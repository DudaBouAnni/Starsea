    ...
    while True:
        ...
        # Inserção
            ...
            if tem_evento.lower() == "s":
                ...
                cursor.execute("SELECT id_evento, nome FROM Eventos")
                eventos = cursor.fetchall()
                for evento in eventos:
                    print(evento)
                id_evento = int(input("Digite o ID do evento desejado: "))
                # Vínculo artista_evento
                comando_evento = "INSERT INTO Evento_Artista (id_artista, id_evento) VALUES (%s, %s)"
            ...
            while genero.lower() == "s":
                ...
                cursor.execute("SELECT * FROM Artista_Genero WHERE id_artista = %s AND id_genero = %s",
        ...
        # Vincular Gênero ao Artista
            ...
            if not cursor.fetchone():
                ...
                return
            cursor.execute(
                "SELECT * FROM Artista_Genero WHERE id_artista = %s AND id_genero = %s",
                (id_artista, id_genero)
            )
            if cursor.fetchone():
                ...
                return
            cursor.execute(
                "INSERT INTO Artista_Genero (id_artista, id_genero) VALUES (%s, %s)",
                (id_artista, id_genero)
            )
            con.commit()
            con.close()
